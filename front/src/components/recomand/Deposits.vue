<template>
    <div>
        ...
        <div>
            <form @submit.prevent="searchDeposits">
                <select v-model="selectedOption">
                    <option v-for=" option in options" :value="option" :key="option.id">
                        {{ option }}
                    </option>
                </select>
                <input type="submit" value="키워드에 따른 예금 상품 추천">
            </form>
        </div>

        <div v-for="(recomand, index)  in recomandDeposit" :key="recomand.id" @click="goDetail(recomand.fin_prdt_cd)">
